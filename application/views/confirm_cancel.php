...
<table class="uk-table uk-table-hover" id="table">
    ...
    <tbody>
        ...
        <tr>
            ...
            <td>
                <form id="confirm_cancel_<?=$user['funds_account']?>" method="post" action="" >
                ...
                <button type='submit' class="uk-button confirm" onclick='getReply("confirm_cancel_<?=$user['funds_account']?>", "irc_<?=$user['funds_account']?>")' id="confirm_button"  name='confirm'>确认</button>    
                <form id="delete_cancel_<?=$user['funds_account']?>" method="post" action="" >
                    <input type='hidden'  id='ird_<?=$user['funds_account']?>' name='reply' >
                    <input type='hidden' value='<?=$user['funds_account']?>' name='id' >
                    <input type='hidden' value='<?=$user['funds_account']?>' name='delete' >
                </form>
                <button class="uk-button delete" onclick='getReply("delete_cancel_<?=$user['funds_account']?>","ird_<?=$user['funds_account']?>")' id="delete_button"  name='delete'>拒绝</button>    
            </td>
